...
def evaluate(config,
    ...
    if return_metrics:
        # metrics as percentages, e.g. {"Recall@1": ..., "Recall@5": ..., "Recall@10": ...,
        #                                "Recall@top1": ..., "AP": ...}
        metrics = {'Recall@{}'.format(i): float(CMC[i-1]*100) for i in ranks}
        metrics['Recall@top1'] = float(CMC[top1]*100)
        metrics['AP'] = float(AP)
        return CMC[0], metrics
